...
fn get_ray_dir(uv: Vec2, pos: Vec3, look_at_pos: Vec3) -> Vec3 {
    let forward = (look_at_pos - 2.0 * pos).normalize();
    let right = vec3(0.5, 2.0, 0.40).cross(forward).normalize();
...
pub fn fs(constants: &ShaderConstants, frag_coord: Vec2) -> Vec4 {
    ...
    let eye_pos = vec3(0.2, 0.0997, 0.2);
    let sun_pos = vec3(-3000.0, 75.0, -1000.0);
